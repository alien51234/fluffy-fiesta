...
async function main() {
    ...
    const trader = new SolanaTrader(connection, senderPrivateKey);
    ...
    const r1 = await trader.newOrder(tokenMintAddress,"BUY", 0.0005, 0.01);
    ...
    // send Token
    const r3 = await trader.send(tokenMintAddress, recipient, 0.1, false)
    console.log("Result 3: " , r3)

    // send SOL
    const r4 = await trader.send(wSOL, recipient, 0.001, false)
